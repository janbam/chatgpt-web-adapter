...
# Crash-safety bound for a writer that exits before acknowledging read terminality.
...
class BrowserNativeBroker:
    ...
    def _request_lease_id(request: dict[str, Any]) -> str | None:
        """Return one non-empty authority lease id from a local request."""

        value = request.get("browserAuthorityLeaseId")
        return value.strip() if isinstance(value, str) and value.strip() else None

    def _expire_authority_reservation(self, lease_id: str) -> None:
        """Release an abandoned post-write reservation after a bounded grace period."""
    ...
    def _reserve_authority_for_readback(self, lease_id: str) -> None:
        """Keep the current lane locked until the matching canonical read arrives."""
    ...
    def _claim_authority_lane(self, operation: str, lease_id: str | None) -> bool:
        """Claim a free lane or continue its matching post-write reservation."""
    ...
    def _complete_authority_reservation(self, lease_id: str) -> str:
        """Release only an existing matching reservation, never a free lane."""
    ...
    def _clear_authority_reservation(self, *, release_lane: bool) -> None:
        """Cancel any reservation and optionally release its retained lane lock."""
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                while True:
                    ...
                    if (
                        ...
                        # Keep the cross-request gap reserved until Python proves
                        # canonical terminality and explicitly acknowledges it.
            ...
            if release_lane and operation == "canonical_read" and lease_id is not None:
                # Timeout and broker-error paths still need Python terminal
                # classification before another write or tab disposal may enter.
                self._reserve_authority_for_readback(lease_id)
                release_lane = False
            if release_lane:
                self.turn_lock.release()
